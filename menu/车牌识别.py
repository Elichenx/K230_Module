...
class LicenceRec:
    ...
    # 绘制车牌检测识别效果
    def draw_result(self,pl,det_res,rec_res,rgb565_img):
#        pl.osd_img.clear()
        if det_res:
            ...
            for det_index in range(len(det_res)):
                for i in range(4):
                    ...
                    point_8[i * 2 + 1] = int(y)
                for i in range(4):
                    rgb565_img.draw_line(point_8[i * 2 + 0],point_8[i * 2 + 1],point_8[(i+1) % 4 * 2 + 0],point_8[(i+1) % 4 * 2 + 1],color=(255, 0, 255, 0),thickness=3)
                rgb565_img.draw_string_advanced( point_8[4], point_8[5] + 60, 20,rec_res[det_index] , color=(255,255,153,18))
...
try:
    ...
    #        print(det_res,rec_res)              # 打印结果
        ...
        show_img = pl.sensor.snapshot(chn=CAM_CHN_ID_1)
        lr.draw_result(pl,det_res,rec_res,show_img)  # 绘制当前帧推理结果
        show_img.draw_string_advanced(0, 0, 15, 'FPS: '+str("%.2f"%(clock.fps())), color=(255, 255, 255))
